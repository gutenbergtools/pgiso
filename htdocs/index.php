<?php
  ...
  //  Modularize so that mysqli_connect is called centrally, not in every
  //   .php file
...
//function displaylist(&$link, &$result)
$iso_size_mb = 0;
...
global $iso_size_bytes; $iso_size_bytes=0;
...
// Note: We will use mysqli_query in MYSQLI_STORE_RESULT mode, whci is the optional third parameter's default:
// mysqli_query($link, "SET NAMES 'utf8'",
...
<html>
 ...
 <h1><font color="red">This tool is temporarily broken, due to a server upgrade problem.  Please check back later.</font></h1>
...
<?php
  ...
  if(isset($_POST['delete_etexts']))
  {
    ...
    print "<h2>displalist 1</h2>";
    displaylist($link, $result);
  }  // if user has clicked the "Add These ETexts" button,
  // the etext entry field is not empty, and the entries are valid
//  else if(isset($_POST['add_etexts']) && !empty($_POST['etexts']) && !preg_match("List etext numbers", $_POST['etexts']))
  else if(isset($_POST['add_etexts']) && !empty($_POST['etexts']) && !preg_match("/List etext numbers/i", $_POST['etexts']))
  {
    $addcount=0;
...
//      if(preg_match("^[A-Za-z0-9\-]+$", $format))
    ...
    foreach($etexts_raw as $author)
    {
      ...
      if(preg_match("/[^A-Za-z\.\ ]/", $author) || preg_match("/^\ *$/", $author) || empty($author))
        continue;
    ...
    foreach($etexts_array as $etext)
    {
      ...
      if(empty($etext) || preg_match("/[^0-9]/", $etext))
        continue;
      ...
      foreach($format_array as $format)
      {
        ...
        // suppress error message from mysqli_num_rows() if query was a delete command
	$duplicate = false;
      ...
      if(mysqli_num_rows($result) && !$duplicate)
      {
        // retrieve id, title, author, format, size and file information to be copied into user's collection
// Need to specify array type, per this guide.  Looks like ASSOC for this:
...
//          while($row = mysqli_fetch_array($result, MYSQLI_ASSOC))
      while ($row = mysqli_fetch_assoc($result))
      {
          if(isset($_SESSION['username']) && isset($_SESSION['collection']))
          {
            // INSERT INTO mysqli_table VALUES (username, coll_id, etext_id, format);
            $query = "INSERT INTO pgiso_perm VALUES ('" . $_SESSION['username'] . "','" . $_SESSION['collection'] .
                     "','" . $row['id'] . "','" . $row['format'] . "','" . date("Y-m-d H:i:s") . "')";
          }
          else
          {
            // INSERT INTO mysqli_table VALUES (session_id, etext_id, format, date);
  ...
  else if(isset($_POST['update_size']))
  {
    // re-calculate the collection ISO size from scratch
       $_SESSION['iso_size_bytes'] = update_iso_size($link, $_SESSION['collection']);
    print "<h2>displalist 4, bytes is $iso_size_bytes</h2>";
    displaylist($link, $result);
  }
...
function displaylist($link, $result)
  {
      ...
      echo "<h1>In displaylist, we have " . mysqli_num_rows($result) . " results</h1>\n";
    ...
    $result = mysqli_query($link, $query);
      echo "<h1>In 2nd displaylist, we have " . mysqli_num_rows($result) . " results</h1>\n";

    // determine the total number of pages by dividing by 25 and rounding up
    $etextstotal = mysqli_num_rows($result);
...
// echo "<!-- " . $query . " -->\n";
    ...
    //  }
    //my $ntotal=0;
    //if (! defined $iso_size_mb) { $ntotal = $iso_size_mb; }
...
//if (number_format($iso_size_mb,2) == 0) {
// print "<font color=\"green\">No etexts added yet.  Please use the space above to add some.</font>\n";
//}

?>

    </form>
      <font size="2">Please send questions and feedback to help2016 at pglaf.org, also refer to the <a href="faq.html">FAQ</a>.</font><br />
    </center>
    <br />
  </body>
</html>
