...
#@{$books{$etext_no}->{'files'}{'txt'}} = grep(!/^[\r\n\ ]+$/, @{$books{$etext_no}->{'files'}{'txt'}});
...
#  }
  ...
  foreach my $key(keys %{$books{$etext_no}->{'files'}})
  {
    ...
    $joined_files = join(";", @{$books{$etext_no}->{'files'}{$key}});
#    $joined_files =~ s/[\r\n\ ]//g;
    if($joined_files !~ m/^[\r\n\ ]*$/)
    {
      $sql = "INSERT INTO test VALUES ('" . $etext_no . "','" . quotemeta($titles) . "','" .
             quotemeta($authors) . "','" . $language . "','" . $key . "','" . $size . "','" .
             $joined_files . "','" . $copyrighted . "')";
